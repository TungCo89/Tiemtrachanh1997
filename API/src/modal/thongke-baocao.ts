...
private validateDates(startDate: string, endDate: string): void {
    ...
    async getDoanhThu(startDate: string, endDate: string): Promise<any> {
        ...
        return await this.thongkeRepository.getDoanhThuByDateRange(startDate, endDate);
    }
    ...
    async getHieuSuatThongKe(startDate: string, endDate: string, topN: number = 10): Promise<any> {
        this.validateDates(startDate, endDate);
        if (topN < 1) {
            topN = 10;
        }
        return await this.thongkeRepository.getHieuSuatThongKe(startDate, endDate, topN);
    }
}
